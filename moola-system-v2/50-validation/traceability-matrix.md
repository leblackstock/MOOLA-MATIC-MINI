...
| `Original-Editable/MOOLA-MATIC IDENTIFY.txt` + `Original-Editable/MOOLA-MATIC MODES.txt` + `Original-Editable/MOOLA-MATIC PROMPT.txt` | Pure Identify default platform recommendations | canonical, conflicting, duplicated | `00-governance/decision-log.md`, `10-canonical/modes/identify.md`, `10-canonical/policies/default-platform-layer.md` | split | split | D-007 approved and implemented: pure Identify excludes platform recommendations by default; the platform layer remains preserved for Value, Identify+Value, and List workflows unless disabled. |
| `Original-Editable/MOOLA-MATIC IDENTIFY.txt` + `templates/identify-template.txt` | Pure Identify-mode contract, identification detail capture, and no-pricing boundary | canonical, duplicated | `10-canonical/modes/identify.md` | preserved | rewrite | Step 1 implemented the missing canonical Identify owner and relocated pure Identify behavior into it without attaching the default platform layer by default. |
...
| `Original-Editable/MOOLA-MATIC VALUE.txt` | Pure Value outward Identify sections and identification context | canonical, conflicting, duplicated | `00-governance/decision-log.md`, `10-canonical/modes/value.md`, `10-canonical/system/run-workflow.md`, `10-canonical/modes/identify.md` | split | split | D-008 approved and implemented: pure Value keeps identification support plus one-sentence identification context, while full outward Identify reporting is no longer the default pure Value contract and combined sequencing now lives in Run execution behavior. |
...
| `Original-Editable/MOOLA-MATIC VALUE.txt` + `Original-Editable/MOOLA-MATIC ESTIMATORS.txt` + `templates/value-template.txt` | Pure Value-mode contract with one-sentence identification context, valuation basis, pricing/profitability stack, and cross-category value drivers | canonical, duplicated, conflicting | `10-canonical/modes/value.md` | split | rewrite | Step 2 implemented the pure Value owner, preserved valuation/profitability behavior, and replaced the default full outward Identify report with one-sentence identification context. |
| `Original-Editable/MOOLA-MATIC PROMPT.txt` + `Original-Editable/MOOLA-MATIC MODES.txt` + `Original-Editable/MOOLA-MATIC IDENTIFY.txt` + `Original-Editable/MOOLA-MATIC VALUE.txt` | Separate default platform recommendation layer outside pure Identify and pure Value contracts | canonical, conflicting, duplicated | `10-canonical/policies/default-platform-layer.md` | split | rewrite | Step 2 implemented the attachment boundary: pure Identify no longer gets the layer by default, while Value, Identify+Value, and List keep platform attachment under the current architecture. |
| `Original-Editable/MOOLA-MATIC ESTIMATORS.txt` + `Original-Editable/MOOLA-MATIC PROMPT.txt` + `templates/measurements/measurements-boots.txt` + `templates/measurements/measurements-shoes.txt` + `templates/measurements/measurements-garments.txt` + `templates/measurements/measurements-standard-lwh.txt` | Cross-category reseller scope with category-specific handling signals and standard fallback measurement behavior | canonical, ambiguous, duplicated | `00-governance/decision-log.md`, `10-canonical/modes/identify.md`, `10-canonical/modes/value.md`, deferred List rewrite surfaces | preserved | rewrite | D-010 approved and partially implemented: Identify and Value now use a universal resale core plus category-specific extension logic, while List-specific cross-category handling remains deferred. |
| `Original-Editable/MOOLA-MATIC LIST.txt` + `Original-Editable/MOOLA-MATIC JSON SCHEMA.txt` + `templates/measurements/measurements-standard-lwh.txt` | List cross-category impact on item specifics, schema expectations, condition language, and title/description dependencies | canonical, ambiguous, automation-required | `00-governance/decision-log.md`, `10-canonical/contracts/listing-json.schema.json`, future List rewrite-planning surfaces | preserved | rewrite | D-010 approved: List impact must be assessed explicitly during the Identify/Value rewrite and not assumed to be footwear/apparel-only. |
...
| `commands/run-moola.md` + `AGENTS.md` + `.cursorrules` + `.cursor/rules/moola-routing.mdc` + `Original-Editable/MOOLA-MATIC MODES.txt` | `/run-moola` default combined Identify+Value behavior for non-List requests | wrapper-only, duplicated, conflicting | `00-governance/decision-log.md`, `10-canonical/system/routing.md`, `10-canonical/system/run-workflow.md`, `30-adapters/commands/run-moola.wrapper.md` | split | split | D-009 approved: `/run-moola` will route by user request and ambiguous requests will use architecture ambiguity-handling rather than auto-running both. |
| `Original-Editable/PROJECT SYSTEM INSTRUCTIONS - MOOLA-MATIC.txt` + `Original-Editable/MOOLA-MATIC PROMPT.txt` + `commands/run-moola.md` | Image input is evidence-first but does not itself force combined Identify+Value behavior | canonical, ambiguous | `00-governance/decision-log.md`, `10-canonical/system/identity-and-scope.md`, `10-canonical/system/routing.md`, `10-canonical/system/run-workflow.md` | rewritten-for-clarity | rewrite | D-009 approved: image input remains evidence input and must not auto-trigger combined Identify+Value behavior by itself. |
| `Original-Editable/MOOLA-MATIC MODES.txt` + `commands/run-moola.md` | Request-based routing, ambiguity handling, and non-default combined Identify+Value behavior under `/run-moola` | canonical, conflicting, duplicated | `10-canonical/system/routing.md`, `10-canonical/system/run-workflow.md` | split | rewrite | Step 3 implemented request-routed selection, explicit ambiguity handling, image-led handling, and sequential combined Identify+Value execution instead of automatic default bundling. |
